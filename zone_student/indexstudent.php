<?php
    ...
    if(!isset($_SESSION['first_pass_id']) || $_SESSION['role'] !== 'student'){
    ...
    $send->bind_param('s',$student_code);
    $send->execute();
...
<body>
    ...
    <div class="container">
            ...
            <div class="logout">
                <a href="indexstudent.php?logout='1'" class="btn-logout">ออกจากระบบ</a>
            </div>
        ...
        <table>
            ...
            <tr class="table-row">
                ...
                <?php if(mysqli_num_rows($result_evaluate) < 1) :?>
                    <td class='last-column last'><a href='evaluateform.php?teacher_id=<?php echo $teacher_id?>&subject_id=<?php echo $subject_id ?>&class=<?php echo urlencode($class)?>' >ประเมิน</a></td>
                <?php else: ?>
                <td class="alr_evaluate last"><div>ประเมินเเล้ว</div></td>
                <?php endif ?>
    ...
    // save evaluate
        if(isset($_SESSION['save'])){
            $save = json_encode($_SESSION['save']);
        
            echo "
            <script type='text/javascript'>
                Swal.fire({
                title: '$save',
                text: 'บันทึกผลการประเมินเรียบร้อยเเล้ว',
                icon: 'success'
                });
            </script>
            ";
    ...
    <script>
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.href = "indexstudent.php?logout='1'";
            }
        })
        });
        });
